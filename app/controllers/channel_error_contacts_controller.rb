  ...
  private
    ...
    def channel_error_contact_params
      if params[:channel_error_contact][:user_id] == "0" or params[:channel_error_contact][:user_id].nil?
        _params = params.require(:channel_error_contact).permit(:success_path, user_attributes: [:id, :email, :role])
        ...
        params.require(:channel_error_contact).permit(:success_path, :user_id)
      end
    end
end
